/******************************
** Name:  [sp_TransformInsertStagedWellOriginUIDData]
** Desc:
** Auth:  Aaron Vogt
** Date:  09/05/2019
**************************
...
*******************************/
CREATE PROCEDURE [DW].[sp_TransformInsertStagedWellOriginUIDData] AS
...
SELECT
	...
	stg.[WellOriginUID]
FROM
	[DW].[vw_DIWellOriginUIDSatStageData] stg
LEFT JOIN
	[DW].[SatWellOriginUID] sat WITH (NOLOCK)
ON
...
IF OBJECT_ID('tempdb..#new_staged_di_well_origin_UID') IS NOT NULL DROP TABLE #new_staged_di_well_origin_UID
...
ON
	...
	sat.WellOriginHashKey IS NOT NULL
...
FROM
	[DW].[SatWellOriginUID] sat
INNER JOIN
	#new_staged_di_well_origin_UID stg
ON
	sat.WellOriginHashKey = stg.WellOriginHashKey AND
...
FROM
	[DW].[vw_DIWellOriginUIDSatStageData] stg
INNER JOIN
	#new_staged_di_well_origin_UID sat
ON
	stg.WellOriginHashKey = sat.WellOriginHashKey AND
	stg.RecordSource = sat.RecordSource AND
