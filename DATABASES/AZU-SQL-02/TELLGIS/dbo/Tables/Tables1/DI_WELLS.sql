CREATE TABLE [dbo].[DI_WELLS] (
    ...
    CONSTRAINT [R528_pk] PRIMARY KEY CLUSTERED ([OBJECTID] ASC) WITH (FILLFACTOR = 75),
    CONSTRAINT [g491_ck] CHECK ([SHAPE].[STSrid]=(3857))
);
...
GO
CREATE NONCLUSTERED INDEX [I1538wellId]
    ON [dbo].[DI_WELLS]([wellId] ASC) WITH (FILLFACTOR = 75);


GO
CREATE NONCLUSTERED INDEX [I1538TD]
    ON [dbo].[DI_WELLS]([TD] ASC) WITH (FILLFACTOR = 75);


GO
CREATE NONCLUSTERED INDEX [I1538operId]
    ON [dbo].[DI_WELLS]([operId] ASC) WITH (FILLFACTOR = 75);


GO
CREATE SPATIAL INDEX [S491_idx]
    ON [dbo].[DI_WELLS] ([Shape])
    WITH  (
            BOUNDING_BOX = (XMAX = -8169373.47821253, XMIN = -19847112.5952422, YMAX = 11700993.0359199, YMIN = 2762292.17145416),
            CELLS_PER_OBJECT = 16
          );
