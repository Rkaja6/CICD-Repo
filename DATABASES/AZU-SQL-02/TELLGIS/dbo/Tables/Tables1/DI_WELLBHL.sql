CREATE TABLE [dbo].[DI_WELLBHL] (
    ...
    CONSTRAINT [R526_pk] PRIMARY KEY CLUSTERED ([OBJECTID] ASC) WITH (FILLFACTOR = 75),
    CONSTRAINT [g489_ck] CHECK ([SHAPE].[STSrid]=(3857))
);
...
GO
CREATE NONCLUSTERED INDEX [I1536wellId]
    ON [dbo].[DI_WELLBHL]([wellId] ASC) WITH (FILLFACTOR = 75);


GO
CREATE NONCLUSTERED INDEX [I1536wellLocatio]
    ON [dbo].[DI_WELLBHL]([wellLocationBottomHoleId] ASC) WITH (FILLFACTOR = 75);


GO
CREATE SPATIAL INDEX [S489_idx]
    ON [dbo].[DI_WELLBHL] ([Shape])
    WITH  (
            BOUNDING_BOX = (XMAX = -8213908.12622183, XMIN = -18465214.3276043, YMAX = 11513118.5718624, YMIN = 2984445.1287111),
            CELLS_PER_OBJECT = 16
          );
