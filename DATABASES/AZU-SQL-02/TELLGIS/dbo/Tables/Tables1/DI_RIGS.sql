CREATE TABLE [dbo].[DI_RIGS] (
    ...
    CONSTRAINT [R523_pk] PRIMARY KEY CLUSTERED ([OBJECTID] ASC) WITH (FILLFACTOR = 75),
    CONSTRAINT [g486_ck] CHECK ([SHAPE].[STSrid]=(3857))
);
...
GO
CREATE NONCLUSTERED INDEX [I1533rig_id]
    ON [dbo].[DI_RIGS]([rig_id] ASC) WITH (FILLFACTOR = 75);


GO
CREATE NONCLUSTERED INDEX [I1533permit_id]
    ON [dbo].[DI_RIGS]([permit_id] ASC) WITH (FILLFACTOR = 75);


GO
CREATE NONCLUSTERED INDEX [I1533formation_d]
    ON [dbo].[DI_RIGS]([formation_depth] ASC) WITH (FILLFACTOR = 75);


GO
CREATE SPATIAL INDEX [S486_idx]
