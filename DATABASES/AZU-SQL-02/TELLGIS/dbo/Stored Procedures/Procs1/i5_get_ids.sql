...
BEGIN
  ...
  BEGIN 
    ...
    UPDATE DBO.i5 WITH  (tablockx, holdlock)
      SET base_id = base_id + @num_requested_ids
      WHERE num_ids = -1 AND id_type = @id_type
    DELETE FROM DBO.i5 WHERE id_type = @id_type and num_ids <> -1
  ...
  BEGIN
    ...
    BEGIN
      DECLARE I_cursor CURSOR FOR
        ...
        FROM DBO.i5 WITH (tablockx, holdlock)
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[i5_get_ids] TO PUBLIC
    WITH GRANT OPTION
    AS [dbo];
