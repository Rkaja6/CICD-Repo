CREATE PROCEDURE DBO.i300_get_ids
@id_type integer,
...
BEGIN
  ...
  BEGIN 
    ...
    UPDATE DBO.i300 WITH  (tablockx, holdlock)
      SET base_id = base_id + @num_requested_ids
      WHERE num_ids = -1 AND id_type = @id_type
    DELETE FROM DBO.i300 WHERE id_type = @id_type and num_ids <> -1
  ...
  BEGIN
    ...
    BEGIN
      DECLARE I_cursor CURSOR FOR
        ...
        FROM DBO.i300 WITH (tablockx, holdlock)
