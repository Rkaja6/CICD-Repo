...
CREATE VIEW [BOLO].[V_AFE_Trans_with_Budget] AS

SELECT 
	A.ACCT AS Account_Code,
	LEFT(A.ACCT,3) AS Major,
	RIGHT(A.ACCT,3) AS Minor,
	MajDes.Name AS Major_Name,
...
FROM 
	...
	FROM [BOLO].[V_AFE_Budget]
	-- Net numbers are the same as gross numbers and on the wrong minor accounts so exclude them
	WHERE COMPANY	= '651'
	) A
LEFT JOIN [BOLO].[Cost_Center] CC
	ON CC.[CC_UID] = A.COST_CENTER
LEFT JOIN [BOLO].[COA] C
	ON C.u2_id = A.ACCT
LEFT JOIN [BOLO].[COA] MajDes
	ON MajDes.u2_id = LEFT(A.ACCT,3)